...
OPERATIONS = {
    ...
    MULTIPLY: Operation(3, [None, None, 1]),
    INPUT: Operation(1, [1]),
    OUTPUT: Operation(1, []),
    JUMP_IF_TRUE: Operation(2, []),
    JUMP_IF_FALSE: Operation(2, []),
    LESS_THAN: Operation(3, [None, None, 1]),
...
def compute(original_input):
    ...
    while True:
        ...
        if opcode == BREAK:
            ...
            inputs[args[2]] = args[0] + args[1]
        elif opcode == MULTIPLY:
            inputs[args[2]] = args[0] * args[1]
        elif opcode == INPUT:
            print("input:")
            user_input = int(input())
            inputs[args[0]] = user_input
        elif opcode == OUTPUT:
            print(args[0])
...
def get_instr(inputs, instr_index):
    ...
    for p_offset in range(1, num_args+1):
        ...
        if immed_mode:
            instr.append(inputs[instr_index + p_offset])
        else:
            instr.append(inputs[inputs[instr_index + p_offset]])
